...
import com.bootcamp.banksim.mod.flux.model.DepositService;
...
public class DepositEndpoint {
  ...
  @PostMapping
  public ResponseEntity register(@RequestBody DepositEntity request) {
    ...
    try {
      ...
      if (request.getAmount() == null || request.getAmount().compareTo(BigDecimal.ZERO) == -1) {
        throw new NullPointerException("El monto de la transacción insertado para crear el depósito no es válido");
      }
      if (request.getSwift() == null || request.getSwift().equals("")) {
        throw new NullPointerException("El swift insertado para crear el depósito no es válido");
      }
      if (request.getIban() == null || request.getIban().equals("")) {
        throw new NullPointerException("El IBAN insertado para crear el depósito no es válido");
      }
      if (!request.getDocumentType().equals("Pasaporte") && !request.getDocumentType().equals("Cédula")) {
        throw new NullPointerException("El tipo de documento insertado para crear el depósito no es válido");
      }
      if (request.getDocumentNum() == null || request.getDocumentNum().equals("")) {
        throw new NullPointerException("El número de documento insertado para crear el depósito no es válido");
      }
      if (request.getIso() == null || request.getIso().equals("")) {
        throw new NullPointerException("El código ISO insertado para crear el depósito no es válido");
      }

      //Checking existence
      BankEntity bank = bankService.findBySwift(request.getSwift());
      if (bank == null) {
        throw new NullPointerException("El banco referenciado para crear el depósito no existe");
      }

      AccountEntity account = accountService.findByIban(request.getIban());
      if (account == null) {
        throw new NullPointerException("La cuenta referenciada para crear el depósito no existe");
      }

      AccountBalanceEntity balance = balanceService.findByAccount(account);
      if (balance == null) {
        throw new NullPointerException("La cuenta referenciada para crear el depósito no tiene un balance asociado");
      }

      CurrencyEntity currency = currencyService.findByIso(request.getIso());
      if (currency == null) {
        throw new NullPointerException("La moneda referenciada para crear el depósito no existe");
      }

      CustomerEntity customer = customerService.findByDocument(request.getDocumentNum(), request.getDocumentType());
      if (customer == null) {
        throw new NullPointerException("El cliente referenciado para crear el depósito no existe");
      }
      ...
      deposit = service.create(request);
      if (deposit == null) {
        throw new NullPointerException("Fallo en la creación del depósito");
      }
      ...
      movement.setTransactionId(deposit.getTransactionId());
      ...
      //Affect Balance
      balance = balanceService.add(balance, deposit.getAmount());
      ...
      //State
      service.updateState(deposit, "Completed");
      ...
      switch (revert) {
        ...
        case MOVEMENT -> {
          service.updateState(deposit, "Reverted");
