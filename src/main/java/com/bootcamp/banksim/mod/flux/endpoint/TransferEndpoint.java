...
//import com.bootcamp.banksim.mod.flux.action.TransferAction;
...
public class TransferEndpoint {
  ...
  //private final TransferAction action = new TransferAction();
  @Autowired
  ...
  @PostMapping
  public ResponseEntity register(@RequestBody TransferEntity request) {
    ...
    try {
      ...
      boolean isSenderBank = bankService.isMyBank(senderBank);
      boolean isReceiverBank = bankService.isMyBank(receiverBank);

      if (isSenderBank && isReceiverBank) {
        transfer = FullTransfer(request);

      } else if (isSenderBank) {
        transfer = Send(request);

      } else if (isReceiverBank) {
        transfer = Receive(request);

      } else {
        throw new NullPointerException("Ninguno de los bancos en la transferencia es este banco");
      }
  ...
  private void Validate(TransferEntity request) {
  ...
  private void Ensure(TransferEntity request) {
    ...
    EnsureBank(senderBank);
    EnsureBank(receiverBank);

    if (bankService.isMyBank(senderBank)) {

      EnsureCurrency(senderCurrency);
      EnsureAccount(senderAccount);
      EnsureBalance(senderBalance);
      EnsureCustomer(senderCustomer);
    }

    if (bankService.isMyBank(receiverBank)) {

      EnsureCurrency(receiverCurrency);
      EnsureAccount(receiverAccount);
      EnsureBalance(receiverBalance);
      EnsureCustomer(receiverCustomer);
    }
  }

  private void Consistence(TransferEntity request) {
  ...
  private TransferEntity Send(TransferEntity request) {
  ...
  private TransferEntity Receive(TransferEntity request) {
  ...
  private TransferEntity FullTransfer(TransferEntity request) {
  ...
  private void Revert(TransferEntity transfer) {
  ...
  private void EnsureBank(BankEntity bank) {

    if (bank == null) {
      throw new NullPointerException("Uno de los bancos referenciados no existe");
    }
  }

  private void EnsureCurrency(CurrencyEntity currency) {

    if (currency == null) {
      throw new NullPointerException("Una de las monedas referenciadas no existe");
    }
  }

  private void EnsureAccount(AccountEntity account) {

    if (account == null) {
      throw new NullPointerException("Una de las cuentas referenciadas no existe");
    }
  }

  private void EnsureBalance(AccountBalanceEntity balance) {

    if (balance == null) {
      throw new NullPointerException("Una de las cuentas referenciadas no tiene un balance asociado");
    }
  }

  private void EnsureCustomer(CustomerEntity customer) {

    if (customer == null) {
      throw new NullPointerException("Uno de los clientes referenciados no existe");
    }
  }
}
